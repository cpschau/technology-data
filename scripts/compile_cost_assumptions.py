...
def get_data_DEA(tech, data_in, expectation=None):
    ...
    if (
        ...
        and (not ("for_carbon_capture_transport_storage" in excel_file))
        and (not ("renewable_fuels" in excel_file))
    ):
        ...
        df_final.index = df_final.index.str.replace("\[", "(", regex=True).str.replace(
            "\]", ")", regex=True
        )
        df_final["unit"] = df_final.rename(
            index=lambda x: x[x.rfind("(") + 1 : x.rfind(")")]
...
def order_data(tech_data):
    ...
    charger = tech_data.loc[
        ("central water tank storage", "Round trip efficiency")
    ].copy()
    charger["further description"] = "efficiency from sqr(Round trip efficiency)"
    charger[years] = charger[years] ** 0.5 * 10
    charger.rename(index={"Round trip efficiency": "efficiency"}, level=1, inplace=True)
    charger.rename(
        index={"central water tank storage": "water tank charger"},
        level=0,
        inplace=True,
    )
    data = pd.concat([data, charger], sort=True)
    charger.rename(
        index={"water tank charger": "water tank discharger"}, level=0, inplace=True
    )
    data = pd.concat([data, charger], sort=True)
